...
def persist_file(upload):
    ...
    if save_path.exists():
        ...
        st.error(f"Failed to save file {upload.name}")
        ...
        if submit:
            if uploaded_file is not None:
                ...
                if vector_store is not None:
                    ...
                    st.success("Vector store successfully initialized.")
                else:
                    st.error("Vector store initialization failed. Please check your Chroma and Ollama setup.")
            else:
...
def init_vector_store():
    ...
    try:
        ...
        st.info("Vector store initialized with the document chunks.")
        return vector_store
...
def get_context_aware_prompt(context_chain: RetrieverOutputLike) -> Runnable:
    ...
    if context_chain is None:
        st.error("Context chain is not initialized")
        return None
...
def get_response(user_query: str) -> str:
    ...
    if context_chain is None:
        return "Error: Context chain initialization failed"
